...
test_labels <- prep_labels(label_data = asci_messy_labels, survey_name = "asci")$truth
...
asci_data <- prep_asci(image_file = asci_cimg, fromfile = FALSE, type = "4pg_combined_scan")$strips
...
#Form training data:
asci_mixed_data <- array_reshape(asci_mixed_data, dim = c(4240, 46, 320, 3))
asci_data <- array_reshape(asci_mixed_data, dim = c(960, 46, 320, 3))
